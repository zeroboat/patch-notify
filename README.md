...
| **구독 관리** | 고객사별 제품 단위 Gmail / Slack 채널 구독 설정; 구독 전용 수신 이메일(SubscriptionEmail) 별도 관리 — 공문 수신 이메일(CustomerEmail)과 독립; 고객사용 구독 링크 발행(만료일 지정) → 고객사가 직접 솔루션 Gmail 구독 토글 및 수신 이메일 관리 |
| **공문 발송** | 솔루션 선택 또는 직접 입력 방식으로 Gmail 공문 발송, 발송 전 이메일 미리보기, Notice Settings에서 로고·헤더 색상·하단 문구 커스터마이징 |
| **Slack 앱** | 고객사 워크스페이스에 앱 설치 → Home Tab에서 구독 직접 설정, 수신 이메일 확인, 패치노트 조회 |
...
──────────────────────────────────────────────────
📌 사용 전 확인해 주세요
• Slack 알림 — 채널에서 /invite @Patch Notify 입력
• 채널 설정 — 설정 변경 버튼에서 알림 채널 선택 후 저장
• 이메일 알림 — 등록된 수신 이메일로만 발송
• 패치노트 조회 — 최근 패치노트 버튼 → DM으로 수신
──────────────────────────────────────────────────
