...
| 기능 | 설명 |
|------|------|
| **로그인 / 권한 관리** | Admin · Manager · Dev · SE · Guest 5단계 역할 기반 접근 제어 (RBAC) |
| **패치노트 관리** | 제품별 버전/기능추가/기능개선/버그수정/특이사항/내부사항 등록·수정·삭제 (내부사항은 외부 발송 및 Guest 미표시); KO/EN 토글로 미리보기 |
| **커스텀 버전** | 등록 시 "커스텀 버전" 체크 시 외부 미배포 처리 — 발행해도 Notion push·고객사 Slack·고객사 이메일 발송을 모두 건너뛰고 사내 Slack 알림만 진행, Guest 계정 목록에서도 비노출 |
| **유틸리티 관리** | 독립 유틸리티(iOS/Android/Web/공통) 패치노트 관리, 다운로드 파일 필수 여부 설정 |
| **파일 첨부** | 하나의 업로드 모달에서 Release/Debug 파일을 각각 드래그 앤 드롭 또는 클릭으로 다중 선택 후 한 번에 업로드; 다운로드(Debug는 Guest 제외), Nextcloud(NAS) 자동 이중 저장 및 공유 링크 생성 |
| **패치노트 발행** | 작성 후 명시적 발행 처리 — 발행 시 구독자에게 자동 알림 전송 (다운로드 필수 유틸리티는 파일 업로드 후 발행 가능) |
| **Notion 동기화** | Notion Markdown API 기반 패치노트 자동 동기화 (변경 감지 + 강제 동기화), Product / Utility 탭 분리 관리; 커스텀 버전은 push 대상에서 제외 |
| **영문 자동 번역** | 패치노트 등록·수정 시 내부 Ollama AI 서버로 한→영 번역 (백그라운드); 번역 결과가 어색할 경우 "영문 수정" 모달에서 한글 원문·번역 상태에 영향 없이 영문만 직접 보정 가능 |
| **고객사 관리** | 고객사 정보 및 솔루션 구독 현황 관리, Google 연락처 CSV 가져오기 |
| **구독 관리** | 고객사별 제품 단위 Gmail / Slack 채널 구독 설정; 구독 전용 수신 이메일(SubscriptionEmail) 별도 관리 — 공문 수신 이메일(CustomerEmail)과 독립; 고객사용 구독 링크 발행(만료일 지정) → 고객사가 직접 솔루션·유틸리티 Gmail 구독 토글 및 수신 이메일 관리; 구독자 관리 화면에서 수신 거부 상태 확인 및 재활성화 |
| **수신 거부** | 패치노트·공문 이메일 하단에 수신자별 고유 링크 삽입 → 클릭한 이메일 주소만 비활성화(다른 수신자에는 영향 없음); 구독자 관리 화면에서 뱃지로 표시되고 재활성화 가능 |
| **공문 발송** | 솔루션 선택 또는 직접 입력 방식으로 Gmail 공문 발송, 발송 전 이메일 미리보기, Notice Settings에서 로고·헤더 색상·하단 문구·이메일 제목 접두사·패치노트 제목 형식(`{product}`/`{version}` 플레이스홀더) 커스터마이징 |
| **Slack 앱** | 고객사 워크스페이스에 앱 설치 → Home Tab에서 Slack 채널 설정 및 패치노트 조회; Gmail 구독은 웹 구독 페이지에서 관리 |
| **발송 로그** | 발송 이력 조회 (유형/채널/상태/고객사/날짜 필터), 실패 원인 상세 확인 |
| **Action Log** | Manager 이상 전용 — 웹 UI에서 수행된 주요 액션(패치노트 발행, 고객사 수정, 구독 변경 등) 이력 조회 |

---

## 역할별 권한

| 역할 | 패치노트 | 고객사/구독 | 공문 발송 | 제품/유틸리티 관리 | Slack 관리 | Action Log | Django Admin |
|------|----------|------------|----------|--------------------|-----------|------------|-------------|
| **Admin** | 읽기/쓰기/수정/삭제/발행 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| **Manager** | 읽기/쓰기/수정/삭제/발행 | ✅ | ✅ | ✅ | ✅ | ✅ | ❌ |
| **Dev** | 읽기/쓰기/수정/삭제/발행 | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| **SE** | 읽기 전용 | ✅ | ❌ | ❌ | ❌ | ❌ | ❌ |
| **Guest** | 읽기 전용 (내부사항·Debug 파일 비공개) | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |

- 최초 배포 시 `.env`의 `DJANGO_SUPERUSER_*`로 생성된 계정은 자동으로 **Admin** 권한을 가집니다.
- **Manager**는 Django Admin 패널을 제외한 모든 서비스 기능을 사용할 수 있습니다.
...
| **Email** | Gmail SMTP (smtplib · CID 이미지 임베딩) |
...
2. **Markdown 파일 캐싱** — Notion Markdown API로 받아온 원본을 Docker volume(`notion_md_data`)에 파일로 저장, 파일 내용 비교로 불필요한 DB 업데이트 방지. Nextcloud(NAS) 연동 시 동일 파일을 NAS에 이중 저장하여 데이터 안정성 확보
...
공문·패치노트 이메일의 로고·헤더 색상·하단 문구·제목 형식은 **Notice Settings 페이지에서 직접 관리**합니다.

1. 서버 기동 후 Management > **Notice Settings** 접속 (Admin / Manager)
2. 상단/하단 로고 이미지 업로드 및 너비(px) 설정 — 로고는 CID 인라인 이미지로 첨부되어 Gmail 등 외부 메일 클라이언트에서도 정상 표시됨
3. 헤더 색상 선택 (컬러 피커)
4. 하단 문구 입력
5. 이메일 제목 접두사 입력 (`[접두사] 제목` 형태로 발송, 기본값 `Patch Notify`)
6. 패치노트 이메일 제목 형식 입력 — `{product}`, `{version}` 플레이스홀더 지원 (예: `{product} {version} Release 안내`)
7. 저장 후 우측 미리보기에서 제품/버전을 선택해 실제 데이터 기준으로 실시간 확인
8. 발송되는 모든 이메일 하단에는 수신자별 고유 수신 거부 링크가 자동 삽입됨
...
> Notion MD 파일은 `notion_md_data`, 알림 로고 이미지는 `notice_logo_files` Docker volume에 영속 저장되므로 컨테이너 재시작 시에도 유지됩니다. `backend`와 `qcluster` 서비스가 모두 로고 파일을 읽어야 하므로 두 서비스 volumes에 동일하게 마운트되어 있습니다.
...
2. Admin/Manager가 서비스 내 Slack Workspaces 페이지에서 워크스페이스 **승인** + **고객사 연결**
3. 고객사 직원이 Slack Home Tab에서 Slack 채널 설정 및 패치노트 조회
4. Gmail 구독(솔루션·유틸리티)은 🌐 구독 설정 버튼으로 웹 페이지에서 관리
...
안녕하세요, *고객사명*님 👋
[🌐 구독 설정]  [💬 Slack 채널 설정]  [📧 수신 이메일]
──────────────────────────────────────────────────
• Gmail 구독 — 🌐 구독 설정으로 웹에서 솔루션·유틸리티별 이메일 알림 설정
• Slack 알림 — 채널에서 /invite @Patch Notify 입력 후 💬 Slack 채널 설정에서 등록
• 패치노트 조회 — 솔루션별 최근 패치노트 버튼 → DM으로 수신
──────────────────────────────────────────────────
📦 솔루션 N개  |  📧 이메일 X개 활성  💬 Slack X개 활성
──────────────────────────────────────────────────
*솔루션명*                              [📄 최근 패치노트]
📧 ✅ 3/5개   |   💬 ❌ 비활성
```
...
웹 서비스 이용자를 위한 기능별 상세 사용법은 [GUIDE.md](./GUIDE.md)를 참고하세요.
